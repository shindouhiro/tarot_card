...
suits.forEach(suit => {
  ranks.forEach(rank => {
    minorArcana.push({
      ...
      image: `https://placehold.co/400x600/2a1b4d/fbbf24?text=${rank.en}+of+${suit.en}`,
      upright: `在${suit.name}的领域中，代表了${rank.name}的正向能量。象征着该元素在生活中的顺畅发展与显化。`,
      reversed: `在${suit.name}的领域中，代表了${rank.name}的受阻状态。象征着能量的内耗、延迟或是使用不当。`
    });
  });
});
